...
eLine = function(dat, size = c(1024, 768), horiz = FALSE,
	...
	calculable=TRUE, showLabel=TRUE, opt = list(),smooth=TRUE) 
{
  library(recharts)
    ...
    for(i in 1:ncol(dat)) {
      ...
      if(is.null(opt$series[[i]]$smooth)) {
        opt$series[[i]]$smooth = smooth
      } else {
        warning('You can set the line smooth ')
      }
    }
...
eArea = function(dat, size = c(1024, 768), horiz = FALSE, stack=NULL,
	title = NULL, subtitle = NULL, title.x = "center", title.y = "top", 
	legend = TRUE, legend.x = "left", legend.y= "top", legend.orient="horizontal", 
	...
	xlab.name = "", xlab.namePosition="start", xlim=NULL,
	ylab=TRUE, ylab.type="value", ylab.data=NULL, ylab.position="left", 
	ylab.name = "", ylab.namePosition="start", ylim=NULL,smooth=TRUE,
	calculable=TRUE, showLabel=TRUE, opt = list()) 
{
  library(recharts)
	...
	if(legend){
	  ...
	  opt$legend = legendSet(  data=colnames(dat), legend.x=legend.x, legend.y=legend.y)
